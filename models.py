...
class Actor(nn.Module):
    def __init__(self, state_size, action_size, hidden_size, activation_function='tanh', log_std_init=-0.5, dropout=0):
        super().__init__()
        ...
        self.log_std = Parameter(torch.full((action_size, ), log_std_init, dtype=torch.float32))

    def forward(self, state):
        mean = self.actor(state)
        policy = Independent(Normal(mean, self.log_std.exp()), 1)
        return policy
...
class ActorCritic(nn.Module):
    """Definition of Soft Actor Critor class"""
    def __init__(self, state_size, action_size, hidden_size, activation_function='tanh', log_std_init=-0.5, dropout=0):
        super().__init__()
        self.actor = Actor(state_size, action_size, hidden_size, activation_function=activation_function, log_std_init=log_std_init, dropout=dropout)
...
class GAILDiscriminator(nn.Module):
    def __init__(self, state_size, action_size, hidden_size, state_only=False, forward_kl=False):
        ...
        self.discriminator = _create_fcnn(state_size if state_only else state_size + action_size, hidden_size, 1,
                                          'tanh')

    def forward(self, state, action):
        D = self.discriminator(
            state if self.state_only else _join_state_action(state, action, self.action_size)).squeeze(dim=1)
        return D

    def predict_reward(self, state, action):
        D = torch.sigmoid(self.forward(state, action))
        h = torch.log(D + 1e-6) - torch.log1p(
            -D + 1e-6)  # Add epsilon to improve numerical stability given limited floating point precision
...
class REDDiscriminator(nn.Module):  # fixed dependant on policy and expert policy
